...
**🧭 Orchestration 编排层** — 将结构化子任务编排为可执行流程，在导航/避障/搜索/跟踪/测绘等技能之间切换，管理场景与传感器模式，跟踪执行状态与依赖关系；运行时会持续读取环境反馈，并根据距离、碰撞、任务完成状态等信息决定继续执行、切换技能、推进任务或请求重规划。
...
- **执行时机**：
  ...
  - 执行阶段：持续接收 `FeedbackSnapshot`，由技能 evaluator 输出 `TaskProgress`
  - 决策阶段：由 `RecoveryPolicy` 选择继续当前技能、切换控制模型、推进任务、重试或请求重规划
- **当前已实现的能力**：
  - 线性任务图构建
  - 子任务状态管理
  - 技能/场景/环境解析
  - 当前激活任务的选择
  - 执行上下文维护与嵌套 `world_state` 合并
  - 运行时反馈记录与关键决策日志
  - 导航过程中的动态避障模型切换
  - 碰撞恢复状态跟踪与重规划请求
...
- `task_graph.py`
  定义 `Subtask`、`TaskGraph`、`ExecutionContext`，是编排层的核心数据结构，并保存反馈历史、决策记录和重规划请求状态
- `planner.py`
  把 LLM 产出的结构化任务 payload 转成可执行的 `TaskGraph`
- `dispatcher.py`
  负责激活当前子任务、更新状态、消费 evaluator 输出并推进到下一个子任务
- `recovery.py`
  根据 `TaskProgress` 选择继续、推进、切换技能、重试或请求重规划
- `feedback.py` / `feedback_adapter.py`
  将仿真环境返回的 `obs / reward / done / info` 标准化为编排层可消费的反馈快照
- `registry.py`
...
当前运行链已经加入面向导航任务的安全接管逻辑：

- **导航中动态避障**：导航技能执行时会持续读取最近障碍物距离，默认阈值为 `8.0m`。当最近障碍物距离小于该阈值时，`navigation` evaluator 会请求切换到 `obstacle_avoidance` 控制模型。
- **保持原导航目标**：这次切换只改变底层控制模型，不把当前导航子任务标记为完成或失败。运行时会保留原导航目标，避障模型继续朝原导航目标推进，因此不会影响任务链中后续单独规划的避障子任务。
- **共享 AUV 状态切换**：`LegacySharedAUVRuntimeAdapter` 在切换模式时同步上一个技能环境的 AUV 状态，避免切换模型时重置仿真位置。
- **碰撞恢复**：当最近障碍物距离小于碰撞阈值时进入恢复状态。恢复期间先执行零动作静止，默认 `400` step；在 `200Hz` 仿真步频下约为 `2s`，随后强制垂直上浮。
- **安全区域判定**：恢复过程持续检查最近障碍物距离，默认超过 `8.0m` 并连续保持一段时间后退出恢复状态；如果超过最大恢复步数仍未脱困，则标记恢复失败。
- **LLM 重规划接口**：恢复失败或任务进入终止失败状态时，编排层会设置 `replan_requested` 和 `replan_reason`，并保留 `world_state`、反馈历史和决策记录，供后续 LLM 根据安全区域状态重新规划。

### 还需要继续完善的部分

- 目前只支持**线性任务链**，还不支持分支、并行和 DAG 依赖
- 已有失败恢复与重规划请求接口，但还没有把 LLM 在线重规划结果自动写回新的 `TaskGraph`
- 已形成“观察 -> 评估 -> 决策 -> 切换/推进”的基础闭环，但 LLM 结合实时反馈的持续重规划仍需完善
- 传感器切换和场景切换目前主要来自任务分解结果，后续可进一步接入实时反馈驱动的动态决策
