...
class RGBCamera(object):
    ...
    def _encode_one(self, buf):
        buf.write(
            struct.pack(">qiii", self.timestamp, self.width, self.height, self.channels)
        )
        for i0 in range(self.height):
            for i1 in range(self.width):
                buf.write(
                    struct.pack(
                        ">%dh" % self.channels, *self.image[i0][i1][: self.channels]
                    )
                )
    ...
    def _decode_one(buf):
        self = RGBCamera()
        self.timestamp, self.width, self.height, self.channels = struct.unpack(
            ">qiii", buf.read(20)
        )
        self.image = []
        for i0 in range(self.height):
            self.image.append([])
            for i1 in range(self.width):
                self.image[i0].append(
                    struct.unpack(">%dh" % self.channels, buf.read(self.channels * 2))
                )
        return self
    ...
    def _get_packed_fingerprint():
        if RGBCamera._packed_fingerprint is None:
            RGBCamera._packed_fingerprint = struct.pack(
                ">Q", RGBCamera._get_hash_recursive([])
            )
