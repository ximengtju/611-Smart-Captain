...
public:
	...
	/**
	 * Must call this method to set the control scheme size after creating this
	 * object
	 */
	void SetControlSchemeSizeInBytes(int size);

	/**
	 * Must call this method to give a reference to SkeletalMesh and an array of
	 * indexed joint names
	 */
	void SetSkeletalMesh(USkeletalMeshComponent* SkeletalMesh, FName Joints[]);

	/**
	 * Fingers are less powerful than joints, after this index we will use
	 * TorqueScalarFingers instead of TorqueScalarMuscles
	 */
	void SetFingerStartIndex(int size);
	void SetJointSizes(int ThreeAxis, int TwoAxis, int OneAxis);

	void SetController(AHolodeckPawnController* const NewController) {
...
private:
	...
	const float TorqueScalarMuscles =
		5.0f; // Muscles can apply more torque than fingers
	 ...
	 * calcTorqueToApply
	 *
	 * Takes the input command, the bone mass, and the specific scalar value and
	 * computes the torque to apply
	 *
	 * CommandValue is between -1.0 and 1.0. It represents the percentage of the
	 * maximum torque to apply.
	 *
	 * The maximum Torque is calculated by multiplying the bone mass and the given
	 * torque scalar. Bone mass is in kg, TorqueScalar is (Neutonmeters/kg)
	 *
	 */
	float
	CalculateTorque(float CommandValue, float BoneMass, float TorqueScalar = 1.0f);
};
