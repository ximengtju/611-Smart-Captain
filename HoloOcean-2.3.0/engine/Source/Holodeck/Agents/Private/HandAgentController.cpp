...
AHandAgentController::AHandAgentController(const FObjectInitializer& ObjectInitializer)
...
void AHandAgentController::AddControlSchemes() {
	...
	this->JointTorqueControlScheme->SetControlSchemeSizeInBytes(
	...
	this->JointTorqueControlScheme->SetFingerStartIndex(
		AHandAgent::NUM_3_AXIS_JOINTS * 3 - 1);

	// Create the floating control scheme too
	this->HandAgentFloatControlScheme = NewObject<UHandAgentMaxTorqueFloat>();
	this->HandAgentFloatControlScheme->SetTorqueControlScheme(
		this->JointTorqueControlScheme);
...
void AHandAgentController::OnPossess(APawn* PawnParam) {
	...
	if (Components.Num() < 1) {
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("Couldn't find USkeletelMeshComponent for PawnParam"));
	}

	SkeletalMeshComponent = Components[0];

	this->JointTorqueControlScheme->SetSkeletalMesh(
		SkeletalMeshComponent, const_cast<FName*>(AHandAgent::Joints));
