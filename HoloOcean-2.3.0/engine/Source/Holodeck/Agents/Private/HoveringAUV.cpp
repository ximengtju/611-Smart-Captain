...
AHoveringAUV::AHoveringAUV() {
	...
	AIControllerClass = LoadClass<AController>(
		NULL, TEXT("/Script/Holodeck.HoveringAUVController"), NULL, LOAD_None, NULL);
	AutoPossessAI = EAutoPossessAI::PlacedInWorld;

	// These values are all pulled from the solidworks file. They're accurate to
	// the physical vehicle, but don't make for a smooth simulation. We clean them
	// up below for a better experience.
	this->Volume = .03554577;
	this->MassInKG = 31.02;
	this->CenterMass = FVector(-5.9, 0.46, -2.82);
	// vector from mesh origin to center of mass,
	// expressed in body frame
	this->CenterBuoyancy = FVector(-5.96, 0.29, -1.85);
	// vector from mesh origin to center of
	// buoyancy, expressed in body frame
	// this->OffsetToOrigin = FVector(-0.7 , -2   , 32	  ); // vector from UE
	// body origin to mesh origin, expressed in UE body frame These values are
	// completely guesstimations
...
void AHoveringAUV::InitializeAgent() {
	...
	// Convert thruster locations to be relative to the center of mass (in mesh
	// frame)
	for (int i = 0; i < 8; i++) {
...
void AHoveringAUV::Tick(float DeltaSeconds) {
	...
	linAccel =
		ClampVector(linAccel, -FVector(AUV_MAX_LIN_ACCEL), FVector(AUV_MAX_LIN_ACCEL));
	...
	angAccel =
		ClampVector(angAccel, -FVector(AUV_MAX_ANG_ACCEL), FVector(AUV_MAX_ANG_ACCEL));
...
void AHoveringAUV::ApplyThrusters(float* const ThrusterArray) {
	...
	for (int i = 0; i < 8; i++) {
		...
		if (i < 4)
			...
			LocalForce = FVector(
				force / UKismetMathLibrary::Sqrt(2),
				force / UKismetMathLibrary::Sqrt(2),
				0);
		else
			LocalForce = FVector(
		...
		// convert Newtons to cN, reverses y axis
		// except UE actually does force in N, so we don't need to convert, so we're
		// scaling up our force arbitrarily... need to go check how often forces use
		// this conversion, and fix all those instances.

		// Apply force in global frame at thruster location
		FRotator bodyToWorld = this->GetActorRotation();
		FVector	 WorldForce = bodyToWorld.RotateVector(
			 LocalForce); // rotate force from body frame to global frame
		FVector ThrusterWorld = RootMesh->GetCenterOfMass()
			+ bodyToWorld.RotateVector(
				thrusterLocations[i]); // get thruster location in global frame
