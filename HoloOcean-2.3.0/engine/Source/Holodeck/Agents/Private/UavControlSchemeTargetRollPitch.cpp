...
UUavControlSchemeTargetRollPitch::UUavControlSchemeTargetRollPitch(
...
void UUavControlSchemeTargetRollPitch::Execute(
	...
	if (Uav == nullptr) {
		...
		if (Uav == nullptr) {
			UE_LOG(
				LogHolodeck,
				Error,
				TEXT("UUAVControlSchemeTargetRollPitch couldn't get Uav reference"));
			return;
		}
	}
	...
	FVector	 EulerRotation = RotatorToEulerInZYX(
		 CurrentRotator); // Get these in local coords in (Z, Y, X) order -
						  // CurrentRotator.Euler() provides (X, Y, Z)
	FVector CurrentGlobalVelocity = ConvertLinearVector(Uav->GetVelocity(), UEToClient);
	FVector LocalAngularVelocity = CurrentRotator.UnrotateVector(
		Uav->RootMesh->GetPhysicsAngularVelocityInDegrees());
	...
	float RollTorqueToApply = RollController.ComputePIDDirect(
		DesiredRoll, CurrentRoll, CurrentRollRate, DeltaSeconds);
	float PitchTorqueToApply = PitchController.ComputePIDDirect(
		DesiredPitch, CurrentPitch, CurrentPitchRate, DeltaSeconds);
	float YawTorqueToApply =
		YawController.ComputePID(DesiredYawRate, CurrentYawRate, DeltaSeconds);
	float HoverThrust = (Uav->RootMesh->GetMass() * GWorld->GetGravityZ())
		/ (cos(DesiredRoll) * cos(DesiredPitch));
	float ThrustToApply =
		AltitudeController.ComputePIDDirect(
			DesiredAltitude, CurrentPositionZ, CurrentGlobalVelocityZ, DeltaSeconds)
		+ HoverThrust;

	// Calculate first-order filter
	float TauRoll = (RollTorqueToApply > CommandArrayFloat[0]) ? UAV_TAU_UP_ROLL
															   : UAV_TAU_DOWN_ROLL;
	float TauPitch = (PitchTorqueToApply > CommandArrayFloat[1]) ? UAV_TAU_UP_PITCH
																 : UAV_TAU_DOWN_PITCH;
	float TauYawRate = (YawTorqueToApply > CommandArrayFloat[2])
		? UAV_TAU_UP_YAW_RATE
		: UAV_TAU_DOWN_YAW_RATE;
	float TauThrust =
		(ThrustToApply > CommandArrayFloat[3]) ? UAV_TAU_UP_FORCE : UAV_TAU_DOWN_FORCE;
	...
	CommandArrayFloat[0] =
		(1 - AlphaRoll) * RollTorqueToApply + AlphaRoll * RollTorqueToApply;
	CommandArrayFloat[1] =
		(1 - AlphaPitch) * PitchTorqueToApply + AlphaPitch * PitchTorqueToApply;
	CommandArrayFloat[2] =
		(1 - AlphaYaw) * YawTorqueToApply + AlphaYaw * YawTorqueToApply;
	CommandArrayFloat[3] =
		(1 - AlphaThrust) * ThrustToApply + AlphaThrust * ThrustToApply;
}

FVector
UUavControlSchemeTargetRollPitch::RotatorToEulerInZYX(const FRotator& Rotator) const {
	...
	float Pitch =
		asin(2.0 * (Quaternion.W * Quaternion.Y - Quaternion.Z * Quaternion.X));
	float Yaw = atan2(
		2.0 * (Quaternion.W * Quaternion.Z + Quaternion.X * Quaternion.Y),
		1.0 - 2.0 * (Quaternion.Y * Quaternion.Z + Quaternion.Z * Quaternion.Z));
