...
AHandAgent::AHandAgent() {
	...
	AIControllerClass = LoadClass<AController>(
		NULL, TEXT("/Script/Holodeck.HandAgentController"), NULL, LOAD_None, NULL);
...
void AHandAgent::ApplyTorques() {
	...
	for (int JointInd = 0; JointInd < NUM_JOINTS; JointInd++) {
		...
		if (CommandArray[ComInd] != 0) {
			float RotForce =
				FMath::Clamp(CommandArray[ComInd], -MAX_TORQUE, MAX_TORQUE);
		...
		SkeletalMesh->AddTorqueInRadians(
			RotQuat.RotateVector(RotationVector), JointName, false);
	}
}

void AHandAgent::ApplyLevitation() {
	float deltaX =
		std::min(CommandArray[AHandAgent::TOTAL_JOINT_DOF], MAX_MOVEMENT_METERS);
	float deltaY =
		std::min(CommandArray[AHandAgent::TOTAL_JOINT_DOF + 1], MAX_MOVEMENT_METERS);
	float deltaZ =
		std::min(CommandArray[AHandAgent::TOTAL_JOINT_DOF + 2], MAX_MOVEMENT_METERS);
...
const FName AHandAgent::BoneNames[] = {
	FName(TEXT("lowerarm_r")),	FName(TEXT("hand_r")),		FName(TEXT("index_01_r")),
	FName(TEXT("index_02_r")),	FName(TEXT("index_03_r")),	FName(TEXT("middle_01_r")),
	FName(TEXT("middle_02_r")), FName(TEXT("middle_03_r")), FName(TEXT("pinky_01_r")),
	FName(TEXT("pinky_02_r")),	FName(TEXT("pinky_03_r")),	FName(TEXT("ring_01_r")),
	FName(TEXT("ring_02_r")),	FName(TEXT("ring_03_r")),	FName(TEXT("thumb_01_r")),
	FName(TEXT("thumb_02_r")),	FName(TEXT("thumb_03_r"))
};

// If you change this number, change the corresponding number in
// RelativeSkeletalPositionSensor.__init__
