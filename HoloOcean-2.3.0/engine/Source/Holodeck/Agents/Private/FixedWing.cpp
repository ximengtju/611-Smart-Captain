...
// Called every frame
void AFixedWing::Tick(float DeltaSeconds) {
	Super::Tick(DeltaSeconds);

	// Convert linear acceleration to force
	FVector linAccel = FVector(CommandArray[0], CommandArray[1], CommandArray[2]);
	linAccel =
		ClampVector(linAccel, -FVector(FW_MAX_LIN_ACCEL), FVector(FW_MAX_LIN_ACCEL));
	...
	angAccel =
		ClampVector(angAccel, -FVector(FW_MAX_ANG_ACCEL), FVector(FW_MAX_ANG_ACCEL));
...
void AFixedWing::EnableDamping() {
	RootMesh->SetLinearDamping(1.0);
	RootMesh->SetAngularDamping(0.75);
}

void AFixedWing::ApplyThrusters(float* const ThrusterArray) {
	/// Iterate through vertical thrusters
	for (int i = 0; i < 8; i++) {
		float force = FMath::Clamp(ThrusterArray[i], -FW_MAX_THRUST, FW_MAX_THRUST);
		...
		FVector ThrusterWorld = RootMesh->GetCenterOfMass()
			+ bodyToWorld.RotateVector(thrusterLocations[i]);
