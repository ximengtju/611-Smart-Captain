...
void UCleanUpTask::ParseSensorParms(FString ParmsJson) {
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		// If it does not have a configuration block, it must be set via world
		// command
		if (HasConfiguration) {
			TArray<float> nums;
			nums.Add(NumTrash);
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("UCleanUpTask::ParseSensorParms:: Unable to parse json."));
	}
}

void UCleanUpTask::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
	...
	AHolodeckGameMode* Game = static_cast<AHolodeckGameMode*>(Target);
	if (TotalTrash == 0) {
		// By default the total trash is zero, so we must keep updating it until it
		// is not zero
