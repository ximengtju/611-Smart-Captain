 ...
 */
template <int N> class HOLODECK_API MultivariateNormal {
...
public:
	...
	void initSigma(TArray<TSharedPtr<FJsonValue>> sigma) {
		verifyf(
			sigma.Num() == N,
			TEXT("Sigma has size %d and should be %d"),
			sigma.Num(),
			N);
	...
	void initCov(std::array<std::array<float, N>, N> cov) {
		...
		if (success) {
			...
			UE_LOG(
				LogHolodeck,
				Warning,
				TEXT("MVN: Encountered a non-positive definite covariance"));
		}
	}
	void initCov(TArray<TSharedPtr<FJsonValue>> cov) {
		...
		if (is2D) {
			...
			for (int i = 0; i < N; i++) {
				...
				verifyf(
					row.Num() == N,
					TEXT("Cov Row has size %d and should be %d"),
					cov.Num(),
					N);
				for (int j = 0; j < N; j++) {
	 ...
	 * Can return as std::array, TArray, FVector (requires N=3), or float
	 * (requires N=1) All functions draw on sampleArray. If cov hasn't been set,
	 * returns 0s
	 */
