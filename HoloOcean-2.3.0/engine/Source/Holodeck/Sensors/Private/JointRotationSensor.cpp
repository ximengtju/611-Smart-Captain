...
void UJointRotationSensor::InitializeSensor() {
	...
	if (this->Parent->IsA(AAndroid::StaticClass())) {
		...
		UE_LOG(
			...
			Fatal,
			TEXT(
				"Error: Tried to use UJointRotationSensor with unknown agent "
				"type."));
	}
...
void UJointRotationSensor::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
	...
	for (int JointInd = 0; JointInd < this->Joints.Num(); JointInd++) {
		...
		FloatBuffer = AddJointRotationToBuffer(
			JointName,
			true,
...
float* UJointRotationSensor::AddJointRotationToBuffer(
	FString JointName,
	bool	Swing1,
	bool	Swing2,
	bool	Twist,
	...
	FConstraintInstance* Constraint =
		SkeletalMeshComponent->FindConstraintInstance(FName(*JointName));
