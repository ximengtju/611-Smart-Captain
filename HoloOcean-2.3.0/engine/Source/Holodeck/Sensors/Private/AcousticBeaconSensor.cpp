...
void UAcousticBeaconSensor::ParseSensorParms(FString ParmsJson) {
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("UAcousticBeaconSensor::ParseSensorParms:: Unable to parse json."));
	}
}

void UAcousticBeaconSensor::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
	FActorComponentTickFunction* ThisTickFunction) {
	// check if your parent pointer is valid, and if the sensor is on. Then get
	// the velocity and buffer, then send the data to it.
	if (Parent != nullptr && bOn) {
		...
		if (fromSensor) {
			...
			FVector	   fromLocationLocal = UKismetMathLibrary::InverseTransformLocation(
				   SensortoWorld, fromLocation);
			...
			// calculate angles
			WaitBuffer[0] =
				UKismetMathLibrary::Atan2(fromLocationLocal.Y, fromLocationLocal.X);
			float dist = UKismetMathLibrary::Sqrt(
				FMath::Square(fromLocationLocal.X)
				+ FMath::Square(fromLocationLocal.Y));
			...
			// If it can't be received, make buffer full of -1s to alert python side
			// of failure
			if (!inRange || !canSee) {
				for (int i = 0; i < 4; i++) {
					WaitBuffer[i] = -1;
