...
void UImagingSonar::ParseSensorParms(FString ParmsJson) {
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("UImagingSonar::ParseSensorParms:: Unable to parse json."));
	}
...
void UImagingSonar::InitializeSensor() {
	...
	if (AzimuthBinScale > AzimuthBins)
		AzimuthBinScale = AzimuthBins;
...
void UImagingSonar::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
	...
	if (TickCounter == 0) {
		...
		for (TArray<Octree*>& bin : foundLeaves) {
			for (Octree* l : bin) {
				...
				if (l->idx.Y == AzimuthBins)
					--l->idx.Y;

				idx = l->idx.Z * AzimuthBins / AzimuthBinScale
					+ l->idx.Y / AzimuthBinScale;
		...
		if (MultiPath) {
			...
			for (TArray<Octree*>& binLeafs : sortedLeaves) {
				if (binLeafs.Num() > 0) {
					// Get first element in this azimuth, elevation bin (ie idx.Y and
					// idx.Z are the same for all of these)
			...
			while (mapSearch.Num() > 0) {
				...
				i_end =
					FGenericPlatformMath::Min(RangeBins, l->idx.X + ClusterSize + 1);
				j_end =
					FGenericPlatformMath::Min(AzimuthBins, l->idx.Y + ClusterSize + 1);
				k_end = FGenericPlatformMath::Min(
					ElevationBins, l->idx.Z + ClusterSize + 1);
				for (int i = i_start; i < i_end; i++) {
					for (int j = j_start; j < j_end; j++) {
						for (int k = k_start; k < k_end; k++) {
							close = mapSearch.Find(FIntVector(i, j, k));
							if (close != nullptr
								&& FVector::DotProduct(l->normal, (*close)->normal)
									> 0.965) {
			...
			ParallelFor(
				...
				[&](int32 i) {
					...
					for (int32 j = 0; j < iterations; j++) {
						...
						// make sure it's still in range (& compute spherical
						// coordinates)
						if (!inRange(&stepper)) {
							thisCluster.Empty();
							return;
						...
						stepper.idx.X =
							(int32)((stepper.locSpherical.X - RangeMin) / RangeRes);
						stepper.idx.Y =
							(int32)((stepper.locSpherical.Y - minAzimuth) / AzimuthRes);
						stepper.idx.Z =
							(int32)((stepper.locSpherical.Z - minElev) / ElevationRes);
						...
						if (hit != nullptr) {
							...
							if (FVector::DotProduct(returnImpact, (*hit)->normalImpact)
								> 0)
								break;
							else {
								thisCluster.Empty();
					...
					// If we did hit something, ray trace the rest of everything in the
					// cluster
					...
					for (Octree* m : thisCluster) {
						...
						t = FVector::DotProduct((*hit)->loc - m->loc, (*hit)->normal)
							/ (FVector::DotProduct(reflection, (*hit)->normal));
						locBounce = m->loc + reflection * t;

						// find return vector
						// TODO: See if any change in accuracy in just using the hit
						// version, should be pretty close angles
						...
						bounce.locSpherical.X +=
							m->locSpherical.X + FVector::Dist(bounce.loc, m->loc);
						bounce.locSpherical.X /= 2;

						// Convert to contribution index
						noise = rNoise.sampleExponential();
						pdf = rNoise.exponentialScaledPDF(noise);
						m->idx.X = (int32)((bounce.locSpherical.X + noise - RangeMin)
										   / RangeRes);
						m->idx.Y =
							(int32)((bounce.locSpherical.Y - minAzimuth) / AzimuthRes);
						m->cos = FVector::DotProduct(returnRay, (*hit)->normalImpact);
						R1 = (m->z - WaterImpedance) / (m->z + WaterImpedance);
						R2 =
							((*hit)->z - WaterImpedance) / ((*hit)->z + WaterImpedance);
						m->val = R1 * R1 * R2 * R2 * m->cos * pdf;

						// TODO: There's a bug this is working around, find it and fix
						// it
						if (m->idx.X < 0)
							m->idx.X = 0;
						if (m->idx.X > RangeBins)
							m->idx.X = RangeBins - 1;
						if (m->idx.Y < 0)
							m->idx.Y = 0;
						if (m->idx.Y > AzimuthBins)
							m->idx.Y = AzimuthBins - 1;

						// DrawDebugPoint(GetWorld(), m->loc, 3, FColor::Red, false,
						// DeltaTime*TicksPerCapture); DrawDebugPoint(GetWorld(),
						// bounce.loc, 3, FColor::Blue, false,
		...
		for (int i = 0; i < RangeBins; i++) {
			...
			for (int j = 0; j < AzimuthBins; j++) {
				...
				scale_total = scale_range
					* (1 + UKismetMathLibrary::Exp(-azimuth * azimuth / std) * 0.5);
		...
		if (AzimuthStreaks == -1 || AzimuthStreaks == 1) {
			...
			for (int i = 0; i < RangeBins; i++) {
				...
				numPerfect = std::accumulate(
					hasPerfectNormal + i * AzimuthBins,
					hasPerfectNormal + (i + 1) * AzimuthBins,
					0);
				numTotal = std::accumulate(
					count + i * AzimuthBins, count + (i + 1) * AzimuthBins, 0);
				avgPerfect = numTotal == 0 ? 0 : (float)numPerfect / (float)numTotal;
				// UE_LOG(LogHolodeck, Warning, TEXT("Avg Perfect %d, %d, %d, %f"), i,
				// numPerfect, numTotal, avgPerfect);
