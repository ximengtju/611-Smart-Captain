	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		if (JsonParsed->HasTypedField<EJson::Array>("MagneticVector")) {
			TArray<TSharedPtr<FJsonValue>> b =
				JsonParsed->GetArrayField("MagneticVector");
			...
			if (b.Num() == 3) {
				...
				UE_LOG(
					...
					Fatal,
					TEXT(
						"UMagnetometerSensor::ParseSensorParms:: MagneticVector "
						"had wrong size."));
			}
		}
	} else {
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("UMagnetometerSensor::ParseSensorParms:: Unable to parse json."));
	}
}
...
void UMagnetometerSensor::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
