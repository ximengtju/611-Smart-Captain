...
void UWaterFogCommand::Execute() {
	UE_LOG(LogHolodeck, Log, TEXT("UWaterFogCommand::Execute fog"));
	if (NumberParams.size() != 5) {
		UE_LOG(
			LogHolodeck,
			Error,
			TEXT(
				"Unexpected argument length found in UWaterFogCommand. Command "
				"not executed."));
		return;
	}
	...
	if (GameTarget == nullptr) {
		UE_LOG(
			...
			TEXT(
				...
				"UWaterFogCommand::Fog not changed."));
		return;
	}
	...
	if (World == nullptr) {
		UE_LOG(
			...
			Warning,
			TEXT(
				"UWaterFogCommand::Execute found world as nullptr. Fog not "
				"changed."));
		return;
	}
	...
	APostProcessVolume* WaterPPV = nullptr;
	for (AActor* actor : FoundPPVolumes) {
		if (actor->Tags.Contains("WaterPPV")) {
			WaterPPV = Cast<APostProcessVolume>(actor);
			break;
		}
	}
	if (!WaterPPV) {
		UE_LOG(LogHolodeck, Warning, TEXT("No 'WaterPPV' found. Fog not changed."));
		return;
	}

	// Ensure blend is fully applied
	WaterPPV->BlendWeight = 1.0f;

	// Access the post process settings
	FPostProcessSettings&		PPSettings = WaterPPV->Settings;
