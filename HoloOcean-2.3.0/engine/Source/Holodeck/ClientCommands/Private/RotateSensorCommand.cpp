...
void URotateSensorCommand::Execute() {
	...
	if (StringParams.size() != 2 || NumberParams.size() != 3) {
		UE_LOG(
			...
			Error,
			TEXT(
				"Unexpected argument length found in URotateSensorCommand. "
	...
	verifyf(
		Agent,
		TEXT("%s: Could not find an agent with that name! %s"),
		*FString(__func__),
		*AgentName);

	verifyf(
		Agent->SensorMap.Contains(SensorName),
		TEXT("%s: Could not find a sensor with that name! %s"),
		*FString(__func__),
		*SensorName);

	UHolodeckSensor* Sensor = Agent->SensorMap[SensorName];

	// Coordinates from the python side come in roll (x), pitch (y), yaw, (z)
	// order
