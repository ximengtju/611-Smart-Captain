...
void UDebugDrawCommand::Execute() {
	if (StringParams.size() != 0 || NumberParams.size() != 12) {
		UE_LOG(
			...
			Error,
			TEXT(
				"Unexpected argument length found in DebugDrawCommand. Command "
				"not executed."));
		return;
	}
	...
	/*
	 NumberParams[0] is the type of debug object to draw. 0:line, 1:arrow,
	 2:box, 3:point NumberParams[1-3] are the start vector NumberParams[4-6] are
	 the end vector NumberParams[7-9] are the RGB color values NumberParams[10]
	 is the size NumberParams[11] is the lifetime (how long it's avalable)
  */
	...
	if (NumberParams[0] == 0)
		DrawDebugLine(
			World, Vec1, Vec2, Color, persistent, lifetime, 0, NumberParams[10]);
	// Draw debug arrow
		...
		DrawDebugDirectionalArrow(
			...
			Vec1,
			Vec2,
			(NumberParams[10] * 10 + Vec1.Dist(Vec1, Vec2)),
			Color,
			persistent,
			lifetime,
			0,
			NumberParams[10]); // First float param is arrow
							   // size, second is thickness
	// Draw debug box
