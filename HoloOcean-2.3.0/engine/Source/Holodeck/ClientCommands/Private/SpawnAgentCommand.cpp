...
void USpawnAgentCommand::Execute() {
	...
	verifyf(
		StringParams.size() == 2 && NumberParams.size() == 7,
		TEXT("%s: Bad Arguments"),
		*FString(__func__));
	...
	if (GameTarget == nullptr) {
		UE_LOG(
			...
			TEXT(
				...
				"SpawnAgentCommand::Execute Cannot spawn agent without this "
				"pointer!"));
		return;
	}
	...
	if (World == nullptr) {
		UE_LOG(
			...
			Warning,
			TEXT(
				"SpawnAgentCommand::Execute found world as nullptr. Unable to "
				"spawn agent!"));
		return;
	}
	...
	// SpawnAgent command is defined in the HolodeckGameMode blueprint class and
	// can only be edited/seen in the blueprint
	AHolodeckAgent* SpawnedAgent =
		GameTarget->SpawnAgent(AgentType, Location, Rotation, AgentName, IsMainAgent);
	verifyf(
		SpawnedAgent,
		TEXT("%s SpawnAgentCommand did not spawn a new Agent."),
		*FString(__func__));
