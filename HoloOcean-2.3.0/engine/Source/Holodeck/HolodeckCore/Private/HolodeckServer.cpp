...
UHolodeckServer::UHolodeckServer() {
	// Warning -- This class gets initialized a few times by Unreal because it is
	// a UObject. DO NOT rely on singleton-qualities in the constructor, only in
	// the Start() function
	bIsRunning = false;
}
...
void UHolodeckServer::Start() {
	...
	if (bIsRunning) {
		UE_LOG(
			LogHolodeck,
			Warning,
			TEXT("HolodeckServer is already running! Bringing it down and up"));
		Kill();
	}
...
#if PLATFORM_WINDOWS
	auto LoadingSemaphore =
		OpenSemaphore(EVENT_ALL_ACCESS, false, *(LOADING_SEMAPHORE_PATH + UUID));
...
#elif PLATFORM_LINUX
	auto LoadingSemaphore =
		sem_open(TCHAR_TO_ANSI(*(LOADING_SEMAPHORE_PATH + UUID)), O_CREAT, 0777, 0);
	if (LoadingSemaphore == SEM_FAILED) {
		LogSystemError("Unable to open loading semaphore");
	}

	LockingSemaphore1 =
		sem_open(TCHAR_TO_ANSI(*(SEMAPHORE_PATH1 + UUID)), O_CREAT, 0777, 1);
	if (LockingSemaphore1 == SEM_FAILED) {
		LogSystemError("Unable to open server semaphore");
	}

	LockingSemaphore2 =
		sem_open(TCHAR_TO_ANSI(*(SEMAPHORE_PATH2 + UUID)), O_CREAT, 0777, 0);
...
void* UHolodeckServer::Malloc(const std::string& Key, unsigned int BufferSize) {
	// If this key doesn't already exist, or the buffer size has changed, allocate
	// the memory.
	...
	if (!Memory.count(Key) || Memory[Key]->Size() != BufferSize) {
		UE_LOG(
			LogHolodeck,
			Log,
			TEXT("Mallocing %u bytes for key %s"),
			BufferSize,
			UTF8_TO_TCHAR(Key.c_str()));
		Memory[Key] = TUniquePtr<HolodeckSharedMemory>(
			new HolodeckSharedMemory(Key, BufferSize, TCHAR_TO_UTF8(*UUID)));
