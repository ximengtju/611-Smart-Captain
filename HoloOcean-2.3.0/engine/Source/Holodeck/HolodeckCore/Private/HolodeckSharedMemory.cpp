...
HolodeckSharedMemory::HolodeckSharedMemory(
	...
	UE_LOG(
		...
		Log,
		TEXT(
			"HolodeckSharedMemory:: Creating file mapping of size %d with "
			"path %s..."),
		BufferSize,
		ANSI_TO_TCHAR(MemPath.c_str()));

	MemFile = CreateFileMapping(
		INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, this->MemSize, WindowsMemPath);
	...
	if (GetLastError() == ERROR_ALREADY_EXISTS) {
		UE_LOG(
			...
			Warning,
			TEXT(
				"HolodeckSharedMemory:: Unable to create MemFile! The path %s "
				"already exists. Hopefully it is the correct size..."),
			ANSI_TO_TCHAR(MemPath.c_str()));
	}
	...
	if (!MemFile) {
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("HolodeckSharedMemory:: Unable to create MemFile! %s"),
			ANSI_TO_TCHAR(msg.c_str()));
	}

	MemPointer =
		static_cast<void*>(MapViewOfFile(MemFile, FILE_MAP_ALL_ACCESS, 0, 0, 0));
	...
	if (!MemPointer) {
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("HolodeckSharedMemory:: Unable to create MemPointer! %s"),
			ANSI_TO_TCHAR(msg.c_str()));
	}
	...
	MemPointer = static_cast<void*>(
		mmap(nullptr, this->MemSize, PROT_READ | PROT_WRITE, MAP_SHARED, MemFile, 0));
...
void HolodeckSharedMemory::LogSystemError(const std::string& errorMessage) {
	std::string errorMsg = errorMessage + " - Error code: " + std::to_string(errno)
