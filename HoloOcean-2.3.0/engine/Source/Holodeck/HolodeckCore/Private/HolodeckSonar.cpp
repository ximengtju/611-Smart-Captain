...
void UHolodeckSonar::ParseSensorParms(FString ParmsJson) {
	...
	// else {
	// 	UE_LOG(LogHolodeck, Fatal, TEXT("UHolodeckSonar::ParseSensorParms::
	...
	sinOffset =
		UKismetMathLibrary::DegSin(FGenericPlatformMath::Min(Azimuth, Elevation) / 2);
}
...
void UHolodeckSonar::initOctree() {
	...
	if (octree == nullptr && TickCounter >= 5) {
		...
		for (auto& agent : Controller->GetServer()->AgentMap) {
			// skip ourselves
			if (agent.Value == this->GetAttachmentRootActor())
				continue;
			AHolodeckBuoyantAgent* bouyantActor =
				static_cast<AHolodeckBuoyantAgent*>(agent.Value);
		...
		if (toMake.Num() != 0)
			GEngine->AddOnScreenDebugMessage(
				...
				FColor::Red,
				FString::Printf(
					TEXT("Premaking %d Octrees, will take some time..."),
					toMake.Num()));
...
void UHolodeckSonar::viewLeaves(Octree* tree) {
	if (tree->leaves.Num() == 0) {
		DrawDebugPoint(
			GetWorld(), tree->loc, 5, FColor::Red, false, .03 * TicksPerCapture);
	} else {
		for (Octree* l : tree->leaves) {
			viewLeaves(l);
		}
...
bool UHolodeckSonar::inRange(Octree* tree) {
	...
	// FVector locLocal =
	// SensortoWorld.InverseTransformPositionNoScale(tree->loc);
	FVector locLocal = SensortoWorld.GetRotation().UnrotateVector(
		tree->loc - SensortoWorld.GetTranslation());

	// check if it's in range
	tree->locSpherical.X = locLocal.Size();
	if (RangeMin + offset - radius >= tree->locSpherical.X
		|| tree->locSpherical.X >= RangeMax + offset + radius)
		return false;
	...
	if (minAzimuth >= tree->locSpherical.Y || tree->locSpherical.Y >= maxAzimuth)
		return false;
	...
	if (UseApprox) {
		...
		tree->locSpherical.Z =
			UKismetMathLibrary::DegAtan2(locLocal.Size2D(), locLocal.Z);
	}
	if (minElev >= tree->locSpherical.Z || tree->locSpherical.Z >= maxElev)
		return false;
...
void UHolodeckSonar::leavesInRange(
	Octree*			 tree,
	TArray<Octree*>& rLeaves,
	float			 stopAt) {
...
void UHolodeckSonar::shadowLeaves() {
	ParallelFor(sortedLeaves.Num(), [&](int32 i) {
		...
		for (int32 j = 0; j < binLeafs.Num(); j++) {
			...
			if (j != binLeafs.Num() - 1) {
				diff = UKismetMathLibrary::Abs(
					jth->locSpherical.X - binLeafs.GetData()[j + 1]->locSpherical.X);
...
FVector
UHolodeckSonar::spherToEuc(float r, float theta, float phi, FTransform SensortoWorld) {
...
void UHolodeckSonar::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
	...
	if (ShowWarning && TicksPerCapture - TickCounter <= 3) {
		GEngine->AddOnScreenDebugMessage(
			...
			FColor::Red,
			FString::Printf(TEXT(
				"Sonar octree generation may slow down simulation, screen may "
				"appear frozen temporarily")));
	}
