...
public:
	 ...
	 */
	virtual void OnPossess(APawn* InPawn) { Super::OnPossess(InPawn); };

	/**
	 * OnUnPossess
	 * Called to Unpossess a pawn
	 */
	virtual void OnUnPossess() { Super::OnUnPossess(); };
	...
	*/
	...
	/**
	 * ExecuteTeleport
	 * Tells the controlled agent to teleport to the location in the shared
	 * memory.
	 */
	virtual void ExecuteTeleport() { check(0 && "You must override ExecuteTeleport"); };

	/**
	 * ExecuteSetState
	 * Sets a new state for the controller agent.
	 */
	virtual void ExecuteSetState() { check(0 && "You must override ExecuteSetState"); };
