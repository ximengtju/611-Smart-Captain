...
///////////////////////////////////////////////////////////////////////////////
	...
	struct JsonValue {
		...
		bool isNumber() const { return getTag() == JSON_NUMBER; }
		bool isBoolean() const {
			return getTag() == JSON_TRUE || getTag() == JSON_FALSE;
		}
		bool isString() const { return getTag() == JSON_STRING; }
		bool isNode() const {
			return getTag() == JSON_ARRAY || getTag() == JSON_OBJECT;
		}
		bool isArray() const { return getTag() == JSON_ARRAY; }
		bool isObject() const { return getTag() == JSON_OBJECT; }
		...
		int toInt(bool* ok = nullptr) const { return (int)toNumber(ok); }
		...
		operator bool() const { return getTag() != JSON_NULL; }
		/** returns true if this object has typeof tag value. */
		bool operator==(JsonTag tag) const { return getTag() == tag; }
		...
		/** overloads @sa child. */
		JsonValue operator()(const char* keyName) const { return child(keyName); }
	...
	protected:
		...
		bool checkType(bool isMatched, bool* ok) const {
			if (ok == 0) {
				assert(
					isMatched
					&& "failed to convert a JsonValue because of type difference.");
				return true;
			}
	...
	struct JsonIterator {
		...
		bool hasNext() const { return p->next != nullptr; }
	 ...
	 * @sa jsonParse().
	 * used internally by jsonParse(). you do not need to manually handle this
	 * object. just keep allocator instance as long as you want to keep JsonValues.
	 */
	class JsonAllocator {
	public:
	 ...
	 * JsonValues are the index tree for actual data on jsonString. allocator object
	 * holds and manage these indices.
	 *
	 * to keep the JsonValue valid, you have to keep both jsonString (data)
	 *  and allocator (instances) alive.
	 *
	 * to avoid calling alloc/free many times (a serious problem on embedded
	 * devices) the allocator is re-usable and you can hold and re-use instance for
	 * parsing as many as jsonStrings you want. JsonAllocator automatically expands
	 *  if more memory is required and frees all memories automatically.
	 *
	 * @param jsonString the JSon string. this buffer will be modified by this
	 * function.
