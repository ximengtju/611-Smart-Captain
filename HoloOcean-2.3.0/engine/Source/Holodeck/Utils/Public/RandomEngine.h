...
class HOLODECK_API URandomEngine : public UObject {
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable)
	void Seed(int32 InSeed) { Engine.seed(InSeed); }
	...
	UFUNCTION(BlueprintCallable)
	float GetUniformFloat() { return std::uniform_real_distribution<float>()(Engine); }
	...
	UFUNCTION(BlueprintCallable)
	bool GetUniformBool() { return (GetUniformIntInRange(0, 1) == 1); }
	...
	UFUNCTION(BlueprintCallable)
	bool GetBoolWithWeight(float Weight) { return (Weight >= GetUniformFloat()); }

	UFUNCTION(BlueprintCallable)
	int32 GetIntWithWeight(const TArray<float>& Weights) {
		return std::discrete_distribution<int32>(
			Weights.GetData(), Weights.GetData() + Weights.Num())(Engine);
	}
	...
	template <typename T> auto& PickOne(const TArray<T>& Array) {
		check(Array.Num() > 0);
		return Array[GetUniformIntInRange(0, Array.Num() - 1)];
	}

	template <typename T> void Shuffle(TArray<T>& Array) {
