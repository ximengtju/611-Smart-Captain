    ...
    if (Dist <= FromSensor->NoiseMaxDistance)
    {
        ...
        if (IsSensorOriented(this, SendToReceive) && IsSensorOriented(FromSensor, ReceiveToSend))
        {
            ...
            else
            {
                ...
                // temp = Hit.GetActor()->GetName() if Hit.GetActor() != None else "None";
                UE_LOG(LogHolodeck, Log, TEXT("Hit Actor = %s"), *temp);
                temp = (FromSensor->Parent != NULL) ? FromSensor->Parent->GetName() : "None";
                // temp = FromSensor->Parent->GetName() if FromSensor->Parent != None else "None";
