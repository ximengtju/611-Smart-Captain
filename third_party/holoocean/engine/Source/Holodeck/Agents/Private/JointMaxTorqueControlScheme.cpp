...
UJointMaxTorqueControlScheme::UJointMaxTorqueControlScheme(
...
void UJointMaxTorqueControlScheme::SetSkeletalMesh(
...
void UJointMaxTorqueControlScheme::SetJointSizes(
	int ThreeAxis,
	int TwoAxis,
	int OneAxis) {
...
void UJointMaxTorqueControlScheme::Execute(
	...
	unsigned TotalJoints =
		this->ThreeAxisJoints + this->TwoAxisJoints + this->OneAxisJoints;
	...
	for (unsigned BoneInd = 0; BoneInd < TotalJoints; BoneInd++) {
		...
		// Set the Scalar depending on whether its a finger joint or large muscle
		// joint
		...
		CommandArrayFloat[ComInd] =
			this->CalculateTorque(InputCommandFloat[ComInd], BoneMass, scalar);
		ComInd++;
...
float UJointMaxTorqueControlScheme::CalculateTorque(
	float CommandValue,
	float BoneMass,
	float TorqueScalar) {
	...
	return CommandValue
		* (BoneMass
		   * TorqueScalar); // See function declaration for explanation of equation
}
