...
UAndroidControlSchemeMaxTorque::UAndroidControlSchemeMaxTorque(
	const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer) {}

void UAndroidControlSchemeMaxTorque::Execute(
	...
	if (Android == nullptr) {
		...
		if (Android == nullptr) {
			UE_LOG(
				LogHolodeck,
				Error,
				TEXT("UAndroidControlSchemeMaxTorque couldn't get Android reference"));
			return;
		}
	}
	...
	for (int BoneInd = 0; BoneInd < Android->NUM_JOINTS; BoneInd++) {
		...
		CommandArrayFloat[ComInd] =
			calcTorqueToApply(InputCommandFloat[ComInd], BoneMass, scalar);
		ComInd++;
...
float UAndroidControlSchemeMaxTorque::calcTorqueToApply(
	float CommandValue,
	float BoneMass,
	float TorqueScalar) {
	CommandValue = FMath::Clamp(CommandValue, MinCommand, MaxCommand);
	return CommandValue * (BoneMass * TorqueScalar); // See function declaration for
													 // explaination of equation
}
