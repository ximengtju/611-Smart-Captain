...
USurfaceVesselControlPD::USurfaceVesselControlPD(
	const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
	, PositionController(SV_POS_P, 0, SV_POS_D)
	, YawController(SV_YAW_P, 0, SV_YAW_D) {}
...
void USurfaceVesselControlPD::Execute(
	...
	if (SurfaceVessel == nullptr) {
		SurfaceVessel =
			static_cast<ASurfaceVessel*>(SurfaceVesselController->GetPawn());
		if (SurfaceVessel == nullptr) {
			UE_LOG(
				LogHolodeck,
				Error,
				TEXT("USurfaceVesselControlPD couldn't get SurfaceVessel reference"));
			return;
		}
	...
	SurfaceVessel->ApplyBuoyancyDragForce();
	...
	FVector AngularVelocity =
		SurfaceVessel->RootMesh->GetPhysicsAngularVelocityInDegrees();
	...
	float	DesiredYaw = UKismetMathLibrary::Atan2(
		  Position.Y - DesiredPosition.Y, Position.X - DesiredPosition.X);
	DesiredYaw = FMath::RadiansToDegrees(DesiredYaw) + 180.0;

	// Compute force & torque to apply
	float tau = YawController.ComputePIDDirect(
