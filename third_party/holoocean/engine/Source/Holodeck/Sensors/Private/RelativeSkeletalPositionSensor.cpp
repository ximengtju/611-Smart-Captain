...
void URelativeSkeletalPositionSensor::InitializeSensor() {
	...
	if (Parent->IsA(AAndroid::StaticClass())) {
		...
		UE_LOG(
			...
			Fatal,
			TEXT(
				"Error: Tried to use URelativeSkeletalPositionSensor with "
				"unknown agent type."));
	}
	...
	for (int i = 0; i < this->Bones.Num(); i++) {
		this->ParentBones.Insert(
			this->SkeletalMeshComponent->GetParentBone(this->Bones[i]), i);
	}

	// Defer this call to the end because we don't know the size of Bones() at the
	// beginning of the method
	Super::InitializeSensor();
}

void URelativeSkeletalPositionSensor::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
	...
	for (int i = 0; i < Bones.Num(); i++) {
		FQuat BoneQ =
			SkeletalMeshComponent->GetBoneQuaternion(Bones[i], EBoneSpaces::WorldSpace);
		FQuat ParentQ = SkeletalMeshComponent->GetBoneQuaternion(
			ParentBones[i], EBoneSpaces::WorldSpace);
