...
void UDVLSensor::ParseSensorParms(FString ParmsJson) {
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("UDVLSensor::ParseSensorParms:: Unable to parse json."));
	}
}
...
void UDVLSensor::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
	...
	if (Parent != nullptr && bOn) {
		...
		FVector Velocity =
			Parent->GetPhysicsLinearVelocityAtPoint(this->GetComponentLocation());
		...
		if (ReturnRange) {
			...
			for (int i = 0; i < 4; i++) {
				...
				bool	   TraceResult = GetWorld()->LineTraceSingleByChannel(
					  Hit, Location, end, ECollisionChannel::ECC_Visibility, QueryParams);
				FloatBuffer[i + 3] = (TraceResult ? Hit.Distance : MaxRange)
					/ 100; // centimeter to meters
			}
