...
void UPressureSensor::InitializeSensor() {
	...
	if (this->Parent->IsA(AAndroid::StaticClass())) {
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("Error: Tried to use UPressureSensor with unknown agent type."));
	}
...
// Called every frame
void UPressureSensor::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
...
void UPressureSensor::OnHit(
	...
	if (JointMap.Contains(Hit.BoneName.ToString()))
		AddHitToBuffer(
			Hit.BoneName.ToString(), HitBoneLocation, NormalImpulse, PrivateData);
}

float* UPressureSensor::AddHitToBuffer(
	FString BoneName,
	FVector HitBoneLocation,
	FVector NormalImpulse,
	float*	Data) {
