...
void UOpticalModemSensor::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
	FActorComponentTickFunction* ThisTickFunction) {
	// check if your parent pointer is valid, and if the sensor is on. Then get
	// the buffer before sending the data to it.
	bool* BoolBuffer = static_cast<bool*>(Buffer);
	BoolBuffer[0] = false;

	if (Parent != nullptr && bOn) {
		UE_LOG(LogHolodeck, Log, TEXT("Optical Modem: before call canTransmit"));
		if (FromSensor) {
			// Add noise to everything
			FromSensor->NoiseMaxDistance =
				FromSensor->MaxDistance + FromSensor->DistanceNoise.sampleFloat();
			FromSensor->NoiseLaserAngle =
				FromSensor->LaserAngle + FromSensor->AngleNoise.sampleFloat();
			NoiseLaserAngle = LaserAngle + AngleNoise.sampleFloat();

			// if someone starting transmitting and we're in range, we'll receive
			UE_LOG(LogHolodeck, Log, TEXT("Optical Modem: Calling CanTransmit"));
			BoolBuffer[0] = this->CanTransmit();

			// Reset FromSensor now that the message has been sent
			FromSensor = NULL;
		}
	}
	...
	if (LaserDebug) {
		DrawDebugCone(
			...
			GetForwardVector(),
			NoiseMaxDistance * 100,
			FMath::DegreesToRadians(NoiseLaserAngle),
			FMath::DegreesToRadians(NoiseLaserAngle),
			DebugNumSides,
			DebugColor,
			false,
			...
			ECC_WorldStatic,
			1.F);
		// DrawDebugLine(GetWorld(), GetComponentLocation(), GetForwardVector() *
		// NoiseMaxDistance * 100, DebugColor, false, .01,ECC_WorldStatic, 1.F);
	}
}
...
bool UOpticalModemSensor::CanTransmit() {
	...
	UE_LOG(
		LogHolodeck,
		Log,
		TEXT("Optical Modem: Dist = %f  MaxDistance = %f"),
		Dist,
		FromSensor->NoiseMaxDistance);
	...
	if (Dist <= FromSensor->NoiseMaxDistance) {
		...
		if (IsSensorOriented(this, SendToReceive)
			&& IsSensorOriented(FromSensor, ReceiveToSend)) {
			...
			if (Range) {
				...
				transmit = false;
				FString temp = "";
				temp = TraceResult ? "True" : "False";
				// temp = "True" if (TraceResult) else ("False");
				UE_LOG(LogHolodeck, Log, TEXT("TraceResult = %s"), *temp);
				temp = (Hit.GetActor() != NULL) ? Hit.GetActor()->GetName() : "None";
				// temp = Hit.GetActor()->GetName() if Hit.GetActor() != None else
				// "None";
				UE_LOG(LogHolodeck, Log, TEXT("Hit Actor = %s"), *temp);
				temp = (FromSensor->Parent != NULL) ? FromSensor->Parent->GetName()
													: "None";
				// temp = FromSensor->Parent->GetName() if FromSensor->Parent != None
				// else "None";
				UE_LOG(LogHolodeck, Log, TEXT("FromSensor->Parent = %s"), *temp);
				UE_LOG(
					LogHolodeck,
					Log,
					TEXT("Optical Modem: Transmit failed due to range"));
				UE_LOG(
					LogHolodeck, Log, TEXT("Optical Modem: Range = %f"), Hit.Distance);
			}
		} else {
			transmit = false;
			UE_LOG(
				LogHolodeck,
				Log,
				TEXT("Optical Modem: Transmit failed due to orientation"));
		}
	} else {
		transmit = false;
		UE_LOG(
			LogHolodeck, Log, TEXT("Optical Modem: Transmit failed due to distance"));
	}
	return transmit;
	// return false;
}

bool UOpticalModemSensor::IsSensorOriented(
	UOpticalModemSensor* Sensor,
	FVector				 LocalToSensor) {
...
void UOpticalModemSensor::ParseSensorParms(FString ParmsJson) {
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("UOpticalModemSensor::ParseSensorParms:: Unable to parse json."));
	}
}
