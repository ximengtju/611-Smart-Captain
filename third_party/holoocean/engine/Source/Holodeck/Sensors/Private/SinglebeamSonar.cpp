...
void USinglebeamSonar::ParseSensorParms(FString ParmsJson) {
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("USinglebeamSonar::ParseSensorParms:: Unable to parse json."));
	}
...
void USinglebeamSonar::InitializeSensor() {
	...
	sinOffset = UKismetMathLibrary::DegSin(
		FGenericPlatformMath::Min(CentralAngle, OpeningAngle) / 2);
}
...
bool USinglebeamSonar::inRange(Octree* tree) {
	...
	if (UseApprox) {
		tree->locSpherical.Z = ATan2Approx(
			UKismetMathLibrary::Sqrt(
				UKismetMathLibrary::Square(locLocal.Y)
				+ UKismetMathLibrary::Square(locLocal.Z)),
			locLocal.X); // OpeningAngle of leaf we are inspecting
	} else {
		tree->locSpherical.Z = UKismetMathLibrary::DegAtan2(
			UKismetMathLibrary::Sqrt(
				UKismetMathLibrary::Square(locLocal.Y)
				+ UKismetMathLibrary::Square(locLocal.Z)),
			locLocal.X);
	}
	if (minOpeningAngle >= tree->locSpherical.Z
		|| tree->locSpherical.Z >= maxOpeningAngle)
		return false;
...
void USinglebeamSonar::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
	...
	if (TickCounter == 0) {
		...
		for (TArray<Octree*>& bin : foundLeaves) {
			for (Octree* l : bin) {
				...
				l->idx.Y =
					(int32)((l->locSpherical.Y - minCentralAngle) / CentralAngleRes);
				l->idx.Z =
					(int32)((l->locSpherical.Z - minOpeningAngle) / OpeningAngleRes);
				// Sometimes we get float->int rounding errors
				if (l->idx.Y == CentralAngleBins)
					--l->idx.Y;
		...
		for (TArray<Octree*>& bin : sortedLeaves) {
			for (Octree* l : bin) {
				...
				l->idx.X =
					(int32)((l->locSpherical.X - RangeMin + range_noise) / RangeRes);
