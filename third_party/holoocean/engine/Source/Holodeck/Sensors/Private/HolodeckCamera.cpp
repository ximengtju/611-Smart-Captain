...
UHolodeckCamera::UHolodeckCamera() {
	UE_LOG(
		LogHolodeck,
		Log,
		TEXT("UHolodeckCamera::UHolodeckCamera() initialization called."));
}
...
void UHolodeckCamera::ParseSensorParms(FString ParmsJson) {
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed)) {
		...
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("UHolodeckCamera::ParseSensorParms:: Unable to parse json."));
	}
...
void UHolodeckCamera::InitializeSensor() {
	...
	if (!SceneCapture) {
		...
		// Handle whatever setup of the SceneCapture that won't likely change across
		// different cameras. (These will be the defaults)
		SceneCapture->PostProcessSettings.bOverride_AutoExposureBias = 1;

		// Higher = brighter captured image. Lower = darker
		// This is a magic number that has been fine tuned to the default worlds. Do
		// not edit without thourough testing.
	...
	// set up everything for the texture that you are using for output. These
	// won't likely change for subclasses. Note: the format should probably be 512
	// x 512 because it must be a square shape, and a power of two. If not, then
	// the TargetTexture will cause crashes.
	...
	// The buffer has got to be an FColor pointer so you can export the pixel data
	// to it.
	this->Buffer = static_cast<FColor*>(Super::Buffer);
	this->ViewportClient = Cast<UHolodeckViewportClient>(GEngine->GameViewport);
}

void UHolodeckCamera::TickSensorComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
