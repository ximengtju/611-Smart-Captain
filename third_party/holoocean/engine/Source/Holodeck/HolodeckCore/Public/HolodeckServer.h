 ...
 * HolodeckGameInstance. HolodeckGameMode calls
 * HolodeckGameInstance::StartServer()
 */
UCLASS()
...
public:
	...
	MakeKey(const std::string& AgentName, const std::string& ItemName) {
		return AgentName + "_" + ItemName;
	}
	static const std::string
	MakeKey(const FString& AgentName, const std::string& ItemName) {
		return std::string(TCHAR_TO_UTF8(*AgentName)) + "_" + ItemName;
	}
	static const std::string
	MakeKey(const FString& AgentName, const FString& ItemName) {
		return std::string(TCHAR_TO_UTF8(*AgentName)) + "_"
			+ std::string(TCHAR_TO_UTF8(*ItemName));
	}
