 ...
 * To function properly, HolodeckSensors must be attached to an agent that is
 *controlled by a holodeck controller. To override, you must implement:
 *		GetDataKey
 *		GetDataLength
 *		TickSensorComponent
...
public:
	 ...
	 */
	virtual void SetAgentAndController(
		AHolodeckPawnControllerInterface* ControllerParam,
		FString							  AgentName);

	/**
	 * BeginPlay
	 * Should only call Super::BeginPlay() and InitializeSensor() all other
	 *initialization code should be called from within that function
	 */
	 ...
	 * Instead, they should override TickSensorComponent(float DeltaTime,
	...
	// Allows you to modify the sensor name in the editor to allow for duplicate
	// sensors on an agent Default SensorName should be set in child before
	// calling Super::InitializeSensor
...
protected:
	...
	virtual int GetNumItems() {
		check(0 && "You must override getNumItems");
		return 0;
	};
	...
	virtual int GetItemSize() {
		check(0 && "You must override getItemSize");
		return 0;
	};

	/**
	 * TickSensorComponent
	 * Mimics Unreal Engine's UActorComponent::TickComponent function. See it's
	 * documentation for what the parameters are. Must be overridden by subclass
