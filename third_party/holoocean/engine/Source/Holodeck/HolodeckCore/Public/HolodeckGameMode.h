...
public:
	...
	// These functions allow the Holodeck to do things which cannot normally be
	// done from pure c++ code
	UFUNCTION(BlueprintImplementableEvent)
	AHolodeckAgent* SpawnAgent(
		const FString&	Type,
	...
	void ExecuteCustomCommand(
		const FString&		   Name,
		const TArray<float>&   NumberParameters,
		const TArray<FString>& StringParameters);
