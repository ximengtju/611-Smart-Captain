...
public:
	...
	/**
	 * Teleport
	 * Instantly moves the agent to target location, with the orientation that was
	 * given If no orientation was given, orientation remains unchanged (see
	 * overloaded function)
	 ...
	 * Teleport
	 * Instantly moves the agent to target location, with the orientation that was
	 * given Orientation remains unchanged
	 * @param NewLocation The location to move to
	 * @return Bool if the teleport was successful.
	 */
	...
	bool SetState(
		...
		const FVector&	NewAngVelocity) override;

	/**
	 * InitializeController
	 * Hooks up everything with the controller. This is normally called in the
	 * beginPlay function, but if you have to manually configure a controller, you
	 * will have to call this function after you do it.
	 */
	 ...
	 * @return Float The maximum acceleration the agent can endure before being
	 * abused. -1 means no limit.
	 */
