...
void UHolodeckSensor::SetAgentAndController(
	AHolodeckPawnControllerInterface* ControllerParam,
	FString							  AgentNameParam) {
...
void UHolodeckSensor::InitializeSensor() {
	...
	if (bOn && Controller != nullptr) {
		UE_LOG(
			LogTemp,
			Warning,
			TEXT("Getting buffer of size %d"),
			GetNumItems() * GetItemSize());
		Buffer = Controller->GetServer()->Malloc(
			UHolodeckServer::MakeKey(AgentName, SensorName + SensorDataKey),
			GetNumItems() * GetItemSize());
	} else {
...
void UHolodeckSensor::TickComponent(
	float						 DeltaTime,
	ELevelTick					 TickType,
