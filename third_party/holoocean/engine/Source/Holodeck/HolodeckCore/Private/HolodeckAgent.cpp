...
AHolodeckAgent::AHolodeckAgent() {
	...
	PrimaryActorTick.TickGroup = TG_PrePhysics; // The tick function will we called
												// before any physics simulation.
	AddTickPrerequisiteActor(GetController());	// The agent's controller will
												// always tick before the agent.
}
...
void AHolodeckAgent::InitializeAgent() {
	...
	if (!InitializeController())
		UE_LOG(
			LogHolodeck,
			Warning,
			TEXT("Couldn't initialize HolodeckPawnController for HolodeckAgent."));

	// Need to initialize this so that collision events will work (OnActorHit
	// won't be called without it) This is needed specifically for the collision
	// sensor.
	if (UPrimitiveComponent* PrimitiveComponent =
			Cast<UPrimitiveComponent>(RootComponent)) {
		...
		UE_LOG(
			...
			Warning,
			TEXT(
				"HolodeckAgent unable to get UPrimitiveComponent. Collision "
				"events disabled."));
	}
...
bool AHolodeckAgent::Teleport(const FVector& NewLocation, const FRotator& NewRotation) {
	UE_LOG(
		LogHolodeck, Log, TEXT("Attempting to teleport HolodeckAgent %s"), *AgentName);
	FHitResult DummyHitResult;
	bool	   bWasSuccessful = this->K2_SetActorLocationAndRotation(
		  NewLocation,
		  NewRotation,
		  false, // will not be blocked by object in between current and new
		  // location.
		  DummyHitResult, // this object is where the hit result is reported, if
		  // teleport can be blocked by objects in between.
		  true // the object will retain its momentum(otherwise the android could
			   // not be teleported).
	  );
	...
	if (bWasSuccessful) {
		UE_LOG(
			LogHolodeck,
			Log,
			TEXT("HolodeckAgent %s teleported successfully"),
			*AgentName);
	} else {
		UE_LOG(
			LogHolodeck,
			Warning,
			TEXT("HolodeckAgent %s did not teleport successfully"),
			*AgentName);
	}
...
bool AHolodeckAgent::SetState(
	const FVector&	NewLocation,
	const FRotator& NewRotation,
	const FVector&	NewVelocity,
	const FVector&	NewAngVelocity) {
	...
	bool	   bWasSuccessful = this->K2_SetActorLocationAndRotation(
		  ...
		  HitResult, // this object is where the hit result is reported, if teleport
					 // can be blocked by objects in between.
		  false);

	UPrimitiveComponent* RootComp = (UPrimitiveComponent*)this->GetRootComponent();

	FRotator RotationNow = RootComp->GetComponentRotation();
	FVector	 AngularVelocityVector = RotationNow.RotateVector(
		 NewAngVelocity); // Rotate from local angles to world angles
...
bool AHolodeckAgent::InitializeController() {
	UE_LOG(
		LogHolodeck,
		Log,
		TEXT("Attempting to initialize controller for HolodeckAgent"));
	...
	if (AgentName == "") {
		UE_LOG(
			...
			Error,
			TEXT(
				"HolodeckAgent doesn't have a name! Must set HolodeckAgent's "
				"name"));
		return false;
	}
	...
	if (HolodeckController == nullptr) {
		UE_LOG(
			LogHolodeck,
			Error,
			TEXT("Couldn't find a HolodeckPawnController for HolodeckAgent"));
		return false;
	} else {
