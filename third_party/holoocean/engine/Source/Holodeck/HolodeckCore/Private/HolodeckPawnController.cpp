...
void AHolodeckPawnController::OnPossess(APawn* InPawn) {
	...
	if (ControlledAgent == nullptr) {
		UE_LOG(
			LogHolodeck,
			Fatal,
			TEXT("HolodeckPawnController attached to non-HolodeckAgent!"));
	}
	...
	UE_LOG(
		LogHolodeck,
		Log,
		TEXT("Pawn Possessed: %s, Controlled by: %s"),
		*InPawn->GetHumanReadableName(),
...
void AHolodeckPawnController::Tick(float DeltaSeconds) {
	...
	if (Server == nullptr) {
		UE_LOG(
			LogHolodeck, Fatal, TEXT("HolodeckPawnController couldn't find server..."));
	}
	...
	ControlSchemes[index]->Execute(
		ControlledAgent->GetRawActionBuffer(), ActionBuffer, DeltaSeconds);
}
...
void AHolodeckPawnController::UpdateServerInfo() {
	...
	UHolodeckGameInstance* Instance =
		static_cast<UHolodeckGameInstance*>(GetGameInstance());
	if (Instance != nullptr)
		Server = Instance->GetServer();
	else
		UE_LOG(
			LogHolodeck, Warning, TEXT("Game Instance is not UHolodeckGameInstance."));
}
...
void AHolodeckPawnController::AllocateBuffers(const FString& AgentName) {
	...
	if (Server != nullptr) {
		...
		void* TempBuffer;
		ActionBuffer = Server->Malloc(TCHAR_TO_UTF8(*AgentName), MaxControlSize);

		TempBuffer = Server->Malloc(
			UHolodeckServer::MakeKey(AgentName, CONTROL_SCHEME_KEY), sizeof(uint8));
		ControlSchemeIdBuffer = static_cast<uint8*>(TempBuffer);

		TempBuffer = Server->Malloc(
			UHolodeckServer::MakeKey(AgentName, TELEPORT_FLAG_KEY), sizeof(uint8));
		ShouldChangeStateBuffer = static_cast<uint8*>(TempBuffer);

		TempBuffer = Server->Malloc(
			UHolodeckServer::MakeKey(AgentName, TELEPORT_COMMAND_KEY),
...
void AHolodeckPawnController::ExecuteTeleport() {
	...
	if (ControlledAgent == nullptr) {
		UE_LOG(
			...
			Warning,
			TEXT(
				"AHolodeckPawnController::ExecuteTeleport: Couldn't get "
				"reference to controlled HolodeckAgent"));
		return;
	}
...
void AHolodeckPawnController::ExecuteSetState() {
	...
	if (ControlledAgent == nullptr) {
		UE_LOG(
			...
			Fatal,
			TEXT(
				"AHolodeckPawnController::ExecuteSetState: Couldn't get "
				"reference to controlled HolodeckAgent"));
		return;
	}
	...
	ControlledAgent->SetState(
		TeleportLocation, NewRotation, NewVelocity, NewAngVelocity);
	*ShouldChangeStateBuffer = 0;
}
