...
void UTeleportCameraCommand::Execute() {
	UE_LOG(
		LogHolodeck,
		Log,
		TEXT("TeleportCameraCommand::Execute teleport camera command"));
	...
	if (StringParams.size() != 0) {
		UE_LOG(
			...
			Error,
			TEXT(
				"Unexpected argument length found in TeleportCameraCommand. "
	...
	// Not actually a roll, pitch, yaw rotation, but where the axis the viewport
	// should be looking along
