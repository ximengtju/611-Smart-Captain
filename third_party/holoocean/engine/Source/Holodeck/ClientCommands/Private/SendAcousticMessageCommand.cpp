...
void USendAcousticMessageCommand::Execute() {
	...
	verifyf(
		StringParams.size() == 4 && NumberParams.size() == 0,
		TEXT("USendAcousticMessageCommand::Execute: Invalid Arguments"));
	...
	verifyf(
		fromAgent,
		TEXT("%s Could not find agent %s"),
		*FString(__func__),
		*fromAgentName);
	verifyf(
		fromAgent->SensorMap.Contains(fromSensorName),
		TEXT("%s Sensor %s not found on agent %s"),
		*FString(__func__),
		*fromSensorName,
		*fromAgentName);
	UAcousticBeaconSensor* fromSensor =
		(UAcousticBeaconSensor*)fromAgent->SensorMap[fromSensorName];
	...
	verifyf(
		toAgent, TEXT("%s Could not find agent %s"), *FString(__func__), *toAgentName);
	verifyf(
		toAgent->SensorMap.Contains(toSensorName),
		TEXT("%s Sensor %s not found on agent %s"),
		*FString(__func__),
		*toSensorName,
		*toAgentName);
	UAcousticBeaconSensor* toSensor =
		(UAcousticBeaconSensor*)toAgent->SensorMap[toSensorName];
