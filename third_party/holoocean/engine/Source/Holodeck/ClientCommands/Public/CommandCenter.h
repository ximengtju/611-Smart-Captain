 ...
 * When creating a new commandcenter object, you must call the init after or it
 * will not function. It requires a valid pointer to the server to be
 * initialized, and the currrent gamemode object to continue running correctly.
 */
...
public:
	 ...
	 * @param InputCommand A pointer to a command. The given pointer will be null
	 ...
	 * If you write to the buffer, make sure to set the shouldreadbufferptr to
	 *true. This should be called by UHolodeckGameMode.
	 ...
	 * It opens a channel for receiving commands in json format. It parses them
	 *and then issues the commands.
	 * @param Server The holodeck server for the game.
	 * @param GameMode The game mode for the instance. This pointer is needed for
	 *giving commands.
	 */
...
private:
	...
	/**
	 *ReadCommandBuffer
	 * Reads the buffer, parses the resulting json, and gives the commands to the
	 *command queue.
	 ...
	 * Reads the buffer, parses the resulting json, and gives the commands to the
	 * command queue. It will not succeed if the json object is not in the ofrmat
	 * expected. At a lower level, it calls GetCommand on every json object that
	 * in an array expected to be a command.
	 ...
	 * Separates the parameters into string parameters and number parameters, then
	 * pushes the command to the Commands array.
	 */
