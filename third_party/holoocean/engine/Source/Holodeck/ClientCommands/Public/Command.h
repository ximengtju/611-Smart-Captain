...
public:
	 ...
	 * Use NumberParams, StringParams, and Target as input variables for the
	 * execute function.
	 */
	...
	/**
	 * Init
	 * This must be called after creating a new command object in order to load it
	 * with the correct data.
	 * @param NumberParamters The numbers (represented as floats) to be used as
	 * parameters for the execute call.
	 * @param StringParameters the strings to be used as parameters for the
	 * execute call.
	 * @param TargetParameter The target of the execute function call. Usually
	 * will be the GameMode pointer.
	 */
	virtual void Init(
		const std::vector<float>&		NumberParameters,
		const std::vector<std::string>& StringParameters,
		AActor* const					TargetParameter);

	/**
	 * GetAgent
	 * This returns a pointer to a HolodeckAgent from the HolodeckServer given an
	 * AgentName
