...
namespace gason {
	...
	public:
		...
		StringBuilder& operator<<(const char* string) { return append(string); }

		StringBuilder& operator<<(int number) {
			int length = snprintf(ibuffer + iindex, ilength - iindex, "%d", number);
			if (length > 0)
				iindex += length;
		...
		StringBuilder& operator<<(double number) {
			int length = snprintf(ibuffer + iindex, ilength - iindex, "%lf", number);
			if (length > 0)
				iindex += length;
	...
	public:
		 ...
		 * @param length length (capacity of buffer).
		 */
		explicit JSonBuilder(char* buffer, size_t length)
			: StringBuilder(buffer, length) {}

		using StringBuilder::emptySpaces;
		using StringBuilder::reset;

		/** checks if the specified buffer is not small and holds the proper result
		 * json string. */
		bool isBufferAdequate() const { return emptySpaces() > 0; }

		/** starts an object by @code { @endcode or @code "name" : { @endcode if name
		 * is not empty. */
		...
		/** starts an array by @code [ @endcode or @code "name" : [ @endcode if name
		 * is not empty. */
