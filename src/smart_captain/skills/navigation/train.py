"""Training entrypoint for the Task1 navigation skill."""
...
from smart_captain.skills.navigation.config import DEFAULT_HYPER_PARAMS, TASK1_SAC_HYPER_PARAMS


DEFAULT_ALGORITHM = "sac"
DEFAULT_TOTAL_TIMESTEPS = 4_000_000
DEFAULT_SAVE_INTERVAL = 100_000
DEFAULT_GYM_ID = "task1-v0"


def make_env(env_config: dict[str, Any] | None = None):
    """Create a Task1 navigation environment."""
    from smart_captain.skills.navigation.env import NavigationEnv

    return NavigationEnv(env_config=env_config or copy.deepcopy(DEFAULT_ENV_CONFIG))


def next_run_dir(base_dir: str | Path, prefix: str = "task1-v0_SAC") -> Path:
    """Return the next numbered run directory, e.g. `task1-v0_SAC_10`."""
...
def train(
    ...
    """Train Task1 navigation with SAC and periodic checkpoint saves."""
    ...
    params = copy.deepcopy(hyper_params or TASK1_SAC_HYPER_PARAMS)
    ...
    if model_load_path is None:
        ...
        model = SAC.load(str(model_load_path), env=env, **load_kwargs)
        model.tensorboard_log = str(tensorboard_log)

    elapsed_timesteps = 0
    checkpoint_base = output_dir / "task1-v0_SAC"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train the Task1 navigation skill.")
    ...
    parser.add_argument("--tb-log-name", default=None)
