...
class RotationSensor(object):
    ...
    def _decode_one(buf):
        self = RotationSensor()
        self.timestamp, self.roll, self.pitch, self.yaw = struct.unpack(
            ">qfff", buf.read(20)
        )
        return self
    ...
    def _get_packed_fingerprint():
        if RotationSensor._packed_fingerprint is None:
            RotationSensor._packed_fingerprint = struct.pack(
                ">Q", RotationSensor._get_hash_recursive([])
            )
