...
class fin:
    ...
    def velocity_in_rotated_plane(self, nu_r):
        ...
        vy_rot = np.sqrt(
            (vy * np.sin(self.angle_rad)) ** 2 + (vz * np.cos(self.angle_rad)) ** 2
        )
    ...
    def tau(self, nu_r, nu):
        ...
        """
        ur = self.velocity_in_rotated_plane(
            nu_r[:3]
        )  # Calculate relative velocity in plane of the fin
    ...
    def calculate_deflection(self, desired_torque, nu_r):
        ...
        if desired_torque[1] != 0 and desired_torque[2] == 0:
            ...
            force = -torque / (
                self.R[0] * -np.cos(self.angle_rad)
            )  # Negated Because it is RxF not R*F
        ...
        required_deflection = np.clip(
            required_deflection, -self.deltaMax, self.deltaMax
        )
