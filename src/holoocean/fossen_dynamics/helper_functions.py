...
T_coord_sys = np.array([[1, 0, 0], [0, -1, 0], [0, 0, -1]])
...
def forceLiftDrag(b, S, CD_0, alpha, U_r, e=0.7):
    ...
    def coeffLiftDrag(b, S, CD_0, alpha, sigma, e=0.7):
        ...
        CL = (1 - sigma) * CL + sigma * 2 * np.sign(alpha) * np.sin(
            alpha
        ) ** 2 * np.cos(alpha)
...
def gvect(W, B, theta, phi, r_bg, r_bb):
    ...
    g = np.array(
        [
            ...
            (r_bg[2] * W - r_bb[2] * B) * sth
            + (r_bg[0] * W - r_bb[0] * B) * cth * cphi,
            -(r_bg[0] * W - r_bb[0] * B) * cth * sphi
            - (r_bg[1] * W - r_bb[1] * B) * sth,
        ]
    )
